...
- **Feature**: push-subscription-sw-ready / `.specs/features/push-subscription-sw-ready/` - **VERIFIED (PASS)**, two Verifier passes; now merged.
- **Phase / Task**: Single-task feature (T1 only), fully closed. Fixes a confirmed production bug: `requestPushSubscription()` (`lib/notifications/client.ts`) called `pushManager.subscribe()` right after `register()` resolved, without waiting for the registration to become active - root-caused via Sentry log analysis (real production session: 5 `subscribed` interleaved with 3 `subscribe_failed`, identical "no active Service Worker" exception each time). Fix: `waitForActiveServiceWorker()` races `navigator.serviceWorker.ready` against a user-confirmed 10s timeout; timeout logs a new `sw_not_ready` outcome (error) instead of ever reaching `subscribe()`. See `.specs/features/push-subscription-sw-ready/validation.md` for the full PASS report.
- **Completed**: spec/design/tasks (`3f75cc8`) + T1 implementation (`62e98bb`) + fix-round-1 test additions (`a51119c`) + Verifier PASS + traceability update (`cb27af5`) + lessons distillation (`85a6cec`) + merged into `development` via PR #11 (`6c2a3f3`).
- **In-progress (file:line)**: none - working tree clean, nothing in flight.
- **Next step**: None pending. `development` now contains every merged feature branch (`push-subscription-sw-ready`, `idle-standby-defense`, `notification-delivery-logging`, `sentry-nextjs-instrumentation`); `main` is fully contained in `development` (`git branch -a --merged development` lists `main`). No local branch is ahead of `development`. Next work starts fresh from `development` with a new feature branch per the skill's branching rule.
- **Blockers**: none.
- **Uncommitted files**: none.
- **Branch**: `development`, clean, up to date with `origin/development`.

**Other state to know on resume**: This session's prior analysis of real Sentry production logs (used to root-cause the SW-ready bug) lives only in chat history, not persisted elsewhere in `.specs/`. The `marmiflix-srh-1`/`marmiflix-redis-1` docker containers (local Upstash-compatible test stack, `docker-compose.test.yml`) may still be running from an earlier session - not started or stopped by this update.
