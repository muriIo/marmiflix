...
- **Feature**: idle-standby / `.specs/features/idle-standby/` - **VERIFIED (PASS)**, two Verifier passes
- **Phase / Task**: Medium-scope feature (Design/Tasks phases skipped per auto-sizing). All 4 execution-plan steps done, plus a follow-up (env-configurable idle timeout), on branch `feat/idle-standby-defense` (9 commits ahead of `development`). First Verifier pass: 7/7 ACs (IDLE-01-07) with `file:line` evidence, 149 unit tests (14 new), typecheck/lint/build clean, 3/3 discrimination-sensor mutations killed. Follow-up added `NEXT_PUBLIC_QUEUE_IDLE_TIMEOUT_SECONDS` (default 180s) to `lib/queue/config.ts`, replacing the hardcoded constant in `app/page.tsx`. A second Verifier pass re-checked that change: gate still 149/149 (no test count drift), refactor of `secondsFromEnv`/`secondsFromString` behavior-preserving, and independently reproduced the client-bundle inlining - confirmed functionally correct in both the set and unset case, with one documented nuance (unset resolves via a runtime `process` shim property read that evaluates to `undefined`, not a build-time literal substitution like the set case - same net result, different mechanism, non-blocking). See `.specs/features/idle-standby/validation.md` (both passes recorded) - verdict PASS.
- **Completed**: all 4 execution-plan steps (commits `f0cc6df` spec, `1fd7752` useQueue enabled option, `983240a` useIdleTimer, `a8efe82` Standby component, `2c087c6` page wiring) + first validation report (`99a6a35`) + env-var follow-up (`47340d1`) + second validation report (`1ac921d`). The page-wiring step was manually verified in a real browser (dev server + local `docker-compose.test.yml` Redis, screenshots, network trace) before committing; the env-var change's client-bundle inlining was independently verified twice (once by the implementer, once by the second Verifier) by inspecting compiled `.next/static/chunks` output.
...
- **Branch**: `feat/idle-standby-defense`, 9 commits ahead of `development` (branched from `development`, not `main` - matches this repo's current working-branch convention) - not merged, not pushed.
